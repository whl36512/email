...
pub static	SQL_DEPOSIT	:	&str= r#"select a from funcs.deposit($1) a"#;
pub static	SQL_DEPOSIT_PARAM	:	&str= r#"{"deposit_id":"DEPOSIT_ID","actual_amount":ACTUAL_AMOUNT,"reference_no":"REFERENCE_NO"}"#;

pub static	DIR_EMAIL_DOWNLOAD		:	&str= "/root/ride_email/download";
pub static	DIR_EMAIL_READY			:	&str= "/root/ride_email/ready" ;
pub static	DIR_EMAIL_PROCESSED		:	&str= "/root/ride_email/processed";
pub static	DIR_EMAIL_ERROR			:	&str= "/root/ride_email/error";
...
pub static REGEX_EMAIL_S2		: &str 
	=r#"^       dkim=pass header.i=@alertsp.chase.com header.s=[^ ]{5,7} header.b=[^ ]{8,10};$"#;

//pub static REGEX_EMAIL_S3		: &str =r#"^       spf=pass \(google.com: domain of srs0=.{4}=.{2}=alertsp.chase.com=no-reply@bounce.secureserver.net designates [0-9.]{10,} as permitted sender\) smtp.mailfrom="SRS0=.{4}=.{2}=alertsp.chase.com=no-reply@bounce.secureserver.net";$"#	;

pub static REGEX_EMAIL_S3		: &str =r#"^       spf=pass \(google.com: domain of .{4}=.{4}=.{2}=alertsp.chase.com=no-reply@bounce.secureserver.net designates [0-9.]{10,} as permitted sender\) smtp.mailfrom=".{4}=.{4}=.{2}=alertsp.chase.com=no-reply@bounce.secureserver.net";$"#	;
...
pub static REGEX_EMAIL_MEMO		: &str	
	=	r#"RD([0-9a-f]{8}[0-9a-f]{4}[0-9a-f]{4}[0-9a-f]{4}[0-9a-f]{12})"#	;  //() is capture group. chase memo field does not allow dashes
//pub static REGEX_EMAIL_MEMO		: &str	
